...
export default function FarmFeast() {
  ...
  const filtered = listings.filter(l => {
    const harvestLabel = formatHarvestDate(l.harvest_date as string);
    const matchFilter = activeFilter === "All" || harvestLabel === activeFilter || (activeFilter === "This week" && harvestLabel.includes("day"));
    const matchSearch = (l.produce_name || "").toLowerCase().includes(search.toLowerCase()) || (l.farmer_name || "").toLowerCase().includes(search.toLowerCase()) || (l.farmer_village || "").toLowerCase().includes(search.toLowerCase());
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-ghost { background: transparent; border: 1.5px solid #2D6A4F; color: #2D6A4F; border-radius: 10px; padding: 7px 14px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: inherit; }
      ...
      <div style={{ background: "#1A3C2A", padding: "0 20px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 22, fontWeight: 700, color: "#fff", letterSpacing: "-0.3px" }}>FarmFeast</span>
      ...
      <div style={{ background: "#2D6A4F", padding: "18px 20px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <p style={{ color: "#fff", fontSize: 15, lineHeight: 1.55 }}>
            Every listing below is a real farmer near Bangalore. They set the price. You know their name, their soil, their harvest date.
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px 20px 0" }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <input
            type="text"
            placeholder="Search produce, farmer or village…"
            value={search}
      ...
      <div style={{ maxWidth: 900, margin: "16px auto 40px", padding: "0 20px", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(270px, 1fr))", gap: 18 }}>
        {filtered.map(l => {
          ...
          const harvestColor = l.harvestDate === "Today" ? "#C0392B" : l.harvestDate === "Tomorrow" ? "#D4A017" : "#2D6A4F";
          return (
            <div key={l.id} className="card" style={{ overflow: "hidden" }} onClick={() => setExpanded(l.id)}>
              ...
              <div style={{ background: "#1A3C2A", padding: "18px 18px 14px", position: "relative" }}>
                ...
                <span className="harvest-badge" style={{ position: "absolute", top: 14, right: 14, background: harvestColor + "22", color: harvestColor, border: `1px solid ${harvestColor}44` }}>
                  {l.harvestDate}
                </span>
                <p style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 18, fontWeight: 700, color: "#fff", marginTop: 8 }}>{l.produce_name}</p>
                <p style={{ color: "#95D5B2", fontSize: 12, marginTop: 3 }}>{l.farmer} · {l.village} · {l.km} km</p>
              </div>
              {/* Card body */}
              <div style={{ padding: "14px 18px 18px" }}>
                ...
                <div>{l.tags.map(t => <span key={t} className="pill">{t}</span>)}</div>
              </div>
            </div>
          );
        })}
              ...
              return (
                <>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                    <div>
                      <p style={{ fontSize: 28 }}>{l.emoji}</p>
                      <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, marginTop: 4 }}>{l.produce}</p>
                    </div>
                  ...
                  <div style={{ background: "#F6F3EE", borderRadius: 12, padding: "14px 16px", marginBottom: 16 }}>
                    ...
                    <p style={{ fontSize: 13, color: "#444", lineHeight: 1.6 }}>{l.story}</p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>
                      {[["Village", l.village], ["Soil", l.soil], ["Farm since", l.since], ["Pesticides", "None"]].map(([k, v]) => (
                        <div key={k}>
            ...
            {Object.entries(cart).map(([id, qty]) => {
              ...
              return (
                <div key={id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0ede8" }}>
                  <div>
                    <p style={{ fontWeight: 600 }}>{l.emoji} {l.produce}</p>
                    <p style={{ fontSize: 12, color: "#888" }}>{l.farmer} · {l.village}</p>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <button className="qty-btn" onClick={() => removeFromCart(+id)}>−</button>
                    <span style={{ fontWeight: 700 }}>{qty}</span>
                    <button className="qty-btn" onClick={() => addToCart(+id)}>+</button>
                    <span style={{ fontWeight: 700, color: "#2D6A4F", minWidth: 52, textAlign: "right" }}>₹{l.price * qty}</span>
                  </div>
                </div>
              );
            })}
